...
test_legal() {
    ...
    if [ "$HTTP_CODE" = "200" ]; then
        ...
        echo "  Countries: $(echo "$BODY" | grep -o '"countriesOfOrigin"' | wc -l) sections"
    else
...
test_file() {
    ...
    echo "  Response: ${RESULT:0:200}..."
    if echo "$RESULT" | grep -qi "signedUrl\|uploadUrl\|FileID\|Location"; then
...
test_indexer() {
    ...
    LOGS=$(kubectl -n $NS_CORE logs deploy/osdu-indexer --tail=50 2>/dev/null | grep -i "listening\|REGISTERED\|subscription\|Started\|ready" | tail -3 || true)
    if [ -n "$LOGS" ]; then
